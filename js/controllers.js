...
.controller('mainCtrl', function($scope,  Storage, Rest) {
  /*
  console.log(Storage.getObject('info'));
  Storage.setObject('info', {
    name: 'Thoughts',
  ...
  $scope.data.person = Storage.getObject('person');
  console.log($scope.data.person);
  if($scope.data.person){
    Rest.login(person.username, person.password, function(){
      console.log('login on start app');
    });
  }
